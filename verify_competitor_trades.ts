...
// ── Competitor trades from screenshots ───────────────────────────────────────
...
const COMPETITOR_TRADES: CompetitorTrade[] = [
...
// ── Types from backtest JSON output ──────────────────────────────────────────
...
interface CoinResult {
  coin: string;
  funding: { signals: number; signals_detail: SignalDetail[] };
  pump: {
    signals: number;
    wins: number;
    winRate: number | null;
...
// ── Runner ────────────────────────────────────────────────────────────────────
function runBacktest(coin: string): CoinResult | null {
  ...
  const data: BacktestResult = JSON.parse(
    require("fs").readFileSync(jsonFile, "utf8"),
  );
...
interface MatchResult {
  ...
  signal?: SignalDetail;
  signalType?: string;
  priceDiff?: number; // % diff between signal entry and competitor entry
}

function findClosestSignal(result: CoinResult, compEntry: number): MatchResult {
  const allSignals: { sig: SignalDetail; type: string }[] = [
  ...
  if (!allSignals.length) return { matched: false };
...
async function main() {
  ...
  console.log(
    `Tolerance: competitor entry within ±${PRICE_TOLERANCE_PCT}% of AltShortBot signal`,
  );
  ...
  for (const trade of COMPETITOR_TRADES) {
    ...
    console.log(
      `   Competitor: Short ${trade.leverage}x @ $${trade.entry} → $${trade.exit}` +
    ...
    if (match.matched && match.signal) {
      ...
      console.log(
        `   ✅ CAUGHT — ${match.signalType} @ $${s.entry.toFixed(6)}`,
      );
      console.log(`      Fired:     ${s.firedAt}`);
      console.log(
        `      Verdict:   ${s.verdict}  (${s.finalPct > 0 ? "+" : ""}${s.finalPct.toFixed(2)}% in 48h)`,
      );
      console.log(
        `      Price gap: ${match.priceDiff!.toFixed(1)}% from competitor entry`,
      );
      console.log(
        `      AltShortBot at 3x: ${(Math.abs(s.finalPct) * 3).toFixed(1)}% ROI`,
      );
    } else if (match.signal) {
      const s = match.signal;
      console.log(
        `   ⚠️  NEAR MISS — closest signal: ${match.signalType} @ $${s.entry.toFixed(6)}`,
      );
      console.log(`      Fired:    ${s.firedAt}`);
      console.log(`      Verdict:  ${s.verdict}`);
      console.log(
        `      Price gap: ${match.priceDiff!.toFixed(1)}% from competitor entry (>${PRICE_TOLERANCE_PCT}%)`,
      );
    } else {
      console.log(
        `   ❌ NOT CAUGHT — no signals fired within ±${PRICE_TOLERANCE_PCT}% of $${trade.entry}`,
      );
    }
  }
  ...
  console.log(`\n${"═".repeat(70)}`);
  console.log(
    `Result: ${caught}/${COMPETITOR_TRADES.length} competitor trades caught by AltShortBot`,
  );
  console.log();
}
