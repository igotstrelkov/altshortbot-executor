...
const SEED_SIGNALS: BuildingSignal[] = [
  { coin: "RAVE",    firedAt: "2026-05-10T00:12:00Z", firedAtMs: new Date("2026-05-10T00:12:00Z").getTime(), entry: 0.8076,  fundingApr: -239.0,   squeeze: 20.6 },
  { coin: "XION",    firedAt: "2026-05-10T05:12:00Z", firedAtMs: new Date("2026-05-10T05:12:00Z").getTime(), entry: 0.1638,  fundingApr: -1446.3,  squeeze: 33.6 },
  { coin: "1000XEC", firedAt: "2026-05-10T09:12:00Z", firedAtMs: new Date("2026-05-10T09:12:00Z").getTime(), entry: 0.0095,  fundingApr: -1338.0,  squeeze: 36.8 },
  { coin: "SNT",     firedAt: "2026-05-10T10:12:00Z", firedAtMs: new Date("2026-05-10T10:12:00Z").getTime(), entry: 0.0115,  fundingApr: -2444.5,  squeeze: 20.1 },
  { coin: "SOLAYER", firedAt: "2026-05-10T13:12:00Z", firedAtMs: new Date("2026-05-10T13:12:00Z").getTime(), entry: 0.1300,  fundingApr: -224.2,   squeeze: 36.7 },
];
...
// ── Table row ─────────────────────────────────────────────────────────────────
function row(
  coin: string, time: string, age: string,
  entry: string, now: string,
  pnl: string, pnl3x: string, funding: string,
  icon: string,
): string {
  return [
    pad(coin,    10),
...
const HEADER = row("Coin", "Fired", "Age", "Entry", "Now", "P&L", "At 3x", "Funding", "");
...
async function main() {
  ...
  if (SEED_MODE) {
    ...
    writeFileSync(LOG_FILE, SEED_SIGNALS.map(s => JSON.stringify(s)).join("\n") + "\n");
  ...
  console.log(`${new Date().toISOString()}  |  window: ${LOOKAHEAD_H}h  |  prices: Bybit`);
  ...
  if (active.length) {
    ...
    let totalPnl = 0, wins = 0, confirmed = 0;
    ...
    for (const s of active) {
      ...
      if (!current) {
        console.log(row(s.coin, time, age, fmtPrice(s.entry), "n/a", "?", "?", s.fundingApr.toFixed(0) + "%", "⏳"));
        continue;
      }
      ...
      console.log(row(
        s.coin, time, age,
        fmtPrice(s.entry), fmtPrice(current),
        fmtPct(pnl), fmtPct(pnl3x),
  ...
  if (SHOW_ALL && expired.length) {
    ...
    if (confirmed > 0) {
      console.log(DIVIDER);
      const avg = totalPnl / confirmed;
      console.log(`  ${wins}/${confirmed} winning  |  avg: ${fmtPct(avg)}  |  avg at 3x: ${fmtPct(avg * 3)}`);
    }
  }
