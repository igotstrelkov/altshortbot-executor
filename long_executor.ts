...
const RISK = {
  ...
  stopLossPct:       0.12,   // 12% stop loss below entry
  maxPositions:      5,
...
async function sendTelegram(msg: string): Promise<void> {
  if (!TELEGRAM_TOKEN || !TELEGRAM_CHAT_ID) { console.log("[telegram]", msg); return; }
  try {
...
interface InstrumentInfo { tickSize: string; qtyStep: string; minQty: string; maxLev: number; }
const instrCache = new Map<string, InstrumentInfo>();

async function fetchInstrumentInfo(coin: string): Promise<InstrumentInfo | null> {
...
function loadPositions(): LongPositionStore {
  ...
  catch { return { open: {}, closed: [], paperEquityUsdt: PAPER_ACCOUNT }; }
}
...
interface QueuedLongSignal {
  coin: string; type: string; firedAt: number; firedAtStr: string;
  entry: number; fundingApr: number; oiChange4h: number;
  confidence: string; queuedAt: number;
}
...
function clearQueue(): void { writeFileSync(LONG_QUEUE_FILE, "[]", "utf8"); }
...
// ─── Trading functions ─────────────────────────────────────────────────────────
async function setLeverage(coin: string, leverage: number, maxLev: number): Promise<boolean> {
...
async function openLong(
  ...
  if (parseFloat(qtyStr) < parseFloat(instr.minQty)) {
    console.log(`  ${coin}: qty ${qtyStr} below minQty ${instr.minQty} — skipping`);
    return null;
  }
  ...
  try {
    ...
    if (res.retCode !== 0) {
      await alertError(`openLong(${coin})`, `retCode ${res.retCode}: ${res.retMsg}`);
      return null;
    }
...
async function closePosition(coin: string, reason: string): Promise<boolean> {
  ...
  try {
    ...
    if (res.retCode !== 0) {
      await alertError(`closePosition(${coin}) — ${reason}`, `retCode ${res.retCode}: ${res.retMsg}`);
      return false;
    }
    return true;
  } catch (e) {
    await alertError(`closePosition(${coin}) — ${reason} — verify on app.bybit.com`, e);
    return false;
  }
}
...
async function fetchLivePositionSize(coin: string): Promise<number> {
  ...
  try {
    const res = await client.getPositionInfo({ category: "linear", symbol: `${coin}USDT` });
    const pos = res.result?.list?.[0];
    return pos ? parseFloat(pos.size ?? "0") : 0;
  } catch { return -1; }
}

async function fetchActualClosePrice(coin: string, fallback: number): Promise<number> {
  if (IS_PAPER) return fallback;
  try {
    const res = await client.getClosedPnL({ category: "linear", symbol: `${coin}USDT`, limit: 1 });
...
async function managePositions(store: LongPositionStore): Promise<void> {
  ...
  for (const [coin, pos] of Object.entries(store.open)) {
    ...
    if (!currentPx) { console.log(`  ${coin}: price unavailable`); continue; }
    ...
    if (trailingHit) {
      ...
      closeReason = "stop";
      closePx     = IS_PAPER ? pos.stopLossPx : await fetchActualClosePrice(coin, currentPx);
    ...
    if (closeReason) {
      ...
      const trade: PaperTrade = {
        coin, openedAt: pos.openedAt, closedAt: nowMs,
        entryPx: pos.entryPx, exitPx: closePx,
        sizeCoin: pos.sizeCoin, pnlUsdc: finalPnlUsdt,
        pnlPct: finalPnlPct, closeReason,
      ...
      console.log(`  ${coin}: closed (${closeReason}) ${finalPnlPct.toFixed(2)}% — $${finalPnlUsdt.toFixed(2)}`);
    }
  }
}
...
async function executeSignal(
  ...
  if (store.open[coin]) { console.log(`  ${coin}: already open — skipping`); return; }

  const instr = await fetchInstrumentInfo(coin);
  if (!instr) { console.log(`  ${coin}: not listed or info unavailable`); return; }
  ...
  const stopPx    = entry * (1 - RISK.stopLossPct);        // ← BELOW entry for longs

  const levOk = await setLeverage(coin, leverage, instr.maxLev);
  if (!levOk) { console.log(`  ${coin}: setLeverage failed — skipping`); return; }
  ...
  store.open[coin] = {
    coin, openedAt: Date.now(), entryPx: entry, sizeCoin,
    notionalUsdc: notional, stopLossPx: stopPx,
    targetPx: entry * 1.25,   // 25% target (informational)
    trailingActive: false, signalType: "BUILDING" as const,
    signalConfidence: (sig.confidence === "HIGH" ? "HIGH" : "MEDIUM") as "HIGH" | "MEDIUM",
    isPaper: IS_PAPER,
  };
...
async function printStatus(store: LongPositionStore): Promise<void> {
  ...
  if (!Object.keys(store.open).length) {
    ...
    console.log(`${"Coin".padEnd(10)} ${"Entry".padStart(12)} ${"Now".padStart(12)} ${"P&L".padStart(8)} ${"Age".padStart(8)}`);
    console.log("─".repeat(54));
    for (const [coin, pos] of Object.entries(store.open)) {
      const px  = await fetchCurrentPrice(coin);
      const pnl = px ? ((px - pos.entryPx) / pos.entryPx * 100).toFixed(2) + "%" : "?";
  ...
  if (recent.length) {
    ...
    for (const t of recent) {
      ...
      console.log(`  ${icon} ${t.coin.padEnd(10)} ${t.pnlPct.toFixed(2)}% (${t.closeReason})`);
    }
  }
  console.log();
}
...
async function main(): Promise<void> {
  ...
  if (IS_STATUS) { await printStatus(store); return; }
  ...
  if (Object.keys(store.open).length > 0) {
    ...
    try { await managePositions(store); }
    catch (e) { await alertError("managePositions", e); }
    savePositions(store);
  }
  ...
  for (const sig of queue) {
    ...
    console.log(`  Processing ${sig.coin} (LONG +${sig.fundingApr.toFixed(0)}% APR)...`);
    try { await executeSignal(sig, store, equity); executed++; }
    catch (e) { await alertError(`executeSignal(${sig.coin})`, e); }
    await sleep(200);
  }
