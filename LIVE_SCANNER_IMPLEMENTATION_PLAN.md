...
const PARAMS = {
  ...
  exhaustMinOiDrop: 3, // OI must have dropped >= 3% for exhaustion to fire
  // Blocks signals where OI is flat (shorts not covering)
  // validated: catches NOT-style false positives with flat OI
...
const MIN_PRICE_USDC = 0.001;  // exclude sub-penny micro-caps (e.g. NOT at $0.0005)
                                // micro-cap tokens have longer squeeze cycles than the
                                // 10h detection window and generate false exhaustion signals
...
async function fetchAllCoins(): Promise<string[]> {
  try {
    ...
    const rawCoins = (data?.result?.list ?? [])
      .filter(s => s.quoteCoin === "USDT")
      .map(s => s.symbol.replace("USDT", ""))
      .filter(c => !EXCLUDE_COINS.has(c))
      .sort();
    if (!rawCoins.length) throw new Error("empty list");

    // Apply minimum price filter — fetch all mids in one call
    const midsRaw = await fetchJSON(`${BB_BASE}/v5/market/tickers?category=linear`)
      as { result?: { list?: { symbol: string; lastPrice: string }[] } };
    const priceMap = new Map((midsRaw?.result?.list ?? [])
      .map(t => [t.symbol.replace("USDT", ""), parseFloat(t.lastPrice)]));
    const coins = rawCoins.filter(c => (priceMap.get(c) ?? 0) >= MIN_PRICE_USDC);
...
oiHistory = rawOI.reverse().map((r) => {
  ...
  return {
    timeMs: parseInt(r.timestamp),
    oiUsd: parseFloat(r.openInterest) * price,
  };
});
```
...
- `|close - prevClose| / prevClose * 100 >= pumpMinPct (19%)`
...
and use `PARAMS.exhaustMaxFundingApr` and `PARAMS.exhaustMinOiDrop` directly
inside the function body. Two changes from the backtest version:

1. Remove `exhaustMaxFunding` parameter — use `PARAMS.exhaustMaxFundingApr`
2. Add OI gate to exhaustion: compute `oiDropPct` when `exhaustMinOiDrop > 0`
   (gate condition: `squeezeMinOiDrop > 0 || exhaustMinOiDrop > 0`)
   Add `exhaustOiOk = PARAMS.exhaustMinOiDrop <= 0 || oiDropPct >= PARAMS.exhaustMinOiDrop`
   to the `isExhausting` check.
...
isExhausting = (
  ...
  AND exhaustOiOk                               // OI must have dropped >= 3% (shorts covering)
)
```
...
if (tc.expect.stateAfter?.lastSqueezePhase !== undefined) {
  if (state.lastSqueezePhase !== tc.expect.stateAfter.lastSqueezePhase) {
    failures.push(
      `State: lastSqueezePhase expected ${tc.expect.stateAfter.lastSqueezePhase}, got ${state.lastSqueezePhase}`,
    );
  }
}
```
...
if (spkState.lastBuildingMinFunding > -500) {
  failures.push(
    `SPK: lastBuildingMinFunding should be <= -500 for TREND_BREAK to fire, got ${spkState.lastBuildingMinFunding}`,
  );
}
```
  ...
  if (isTrendBreak || allowNormal) {
    ...
    const hoursSinceExhaustion =
      newState.lastExhaustionMs !== null
        ? (ts - newState.lastExhaustionMs) / 3_600_000
        : Infinity;
...
function checkGate2(
  ...
  const passes =
    oiChangePct >= PARAMS.minOiChangePct && // OI rising
